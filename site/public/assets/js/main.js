...
(function () {
  ...
  function initHotspots() {
    ...
    function restart(a) {                       // replay the drawing from its first frame
    ...
    spots.forEach(function (a) {
      if (!coarse) {
        a.addEventListener("mouseenter", function () { restart(a); });
        a.addEventListener("focus", function () { restart(a); });
      } else {
        a.addEventListener("click", function (e) {
          if (a.dataset.going) return;
          e.preventDefault();
          spots.forEach(function (o) { o.classList.remove("on"); });
          restart(a);
          a.classList.add("on");
          a.dataset.going = "1";
          setTimeout(function () { window.location.href = a.getAttribute("href"); }, reduce ? 120 : 800);
        });
      }
    });
  }
  ...
  function initMenu() {
    ...
    function set(open) {
      ...
      btn.setAttribute("aria-expanded", open ? "true" : "false");
      btn.style.backgroundImage = open ? "url(/assets/icons/close.png)" : "url(/assets/icons/menu.png)";
    }
    ...
    window.addEventListener("resize", function () { if (window.innerWidth >= 680) set(false); });
  ...
  function initSideScroll() {
    ...
    function update() {
      ...
      if (scrollable < 40) { btn.classList.remove("show", "at-end"); return; }  // nothing to scroll
  ...
  document.addEventListener("DOMContentLoaded", function () {
    initHotspots(); initLangDropdown(); initMenu(); initSideScroll(); initBack();
  });
})();
